...
int main()
{
    ...
    const size_t grid_size1 = num_chk / block_size * chk_size;
    const size_t grid_size2 =totMidByteNum/block_size;
    printf ("grid_size2=%zu\n" , grid_size2);
    printf ("totMidByteNum=%zu\n" , totMidByteNum);
    ...
    // // for(int i=0;i<1023;i++) // all magic numbers
    // // {	
	// //   init_tab[i] = (i+1989.64)/(i-989.145);
	// //   init_tab1[i] = (i-199.64)/(i+99.15);
	// //   init_tab2[i] = (i+599.699)/(i-211.985);
    // // }
	...
	//     oriData[i] = (init_tab[i%2023] + init_tab1[2%1024] + init_tab2[i%2025])/3;
    // }
    ...
    printf("...................\n");
    size_t offset = 8*chk_size;
    for(size_t i =offset;i<peek_num+offset;i++)
    {
        printf("%x ",compBuffer_dummy[midByteOffsetPacked+i]);
    }
    printf("...................\n");
    for(size_t i =totMbytes-peek_num; i< totMbytes;i++)
    {
        printf("%x ",compBuffer_dummy[midByteOffsetPacked+i]);
    }
    ...
    for(size_t i =num_chk-1;i<num_chk;i++)
    {
        printf("chunk%d residue bits: ",i);
        for (int k=0;k<10; k++)
        {
